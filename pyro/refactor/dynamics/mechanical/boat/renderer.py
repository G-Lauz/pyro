...
from pyro.refactor.dynamics.mechanical.boat.geometry import BoatGeometry


class BoatRenderer(Renderer):
    def __init__(self, geometry: BoatGeometry, screen_size=(800, 600)):
        ...
        self.pygame.display.set_caption("Pyro - 2D Boat Simulation")
    ...
    def _draw_grid(self, grid_points, vertical_len, horizontal_len):
        for i in range(vertical_len):
            ...
            self.pygame.draw.line(self.screen, self.grid_color, grid_points[start_idx], grid_points[end_idx], 1)
    ...
    def arrow_points(self, inputs):
        ...
        """
        force_magnitude = numpy.linalg.norm(inputs) / 1000  # TODO: fix rendering patch that scale to kN
    ...
    def render_info(self, states : MechanicalStateSignal, inputs: Signal):
        """
        ...
        text = self.font.render(f"Force: {inputs.values}", True, (0, 0, 0))
...
class FixedCameraBoatRenderer(BoatRenderer):
    ...
    def get_vector_force(self, input_force):
    ...
    def render(self, states: MechanicalStateSignal, inputs: Signal, outputs: Signal):
        self.screen.fill(self.background_color)

        positions = states.position
        velocities = states.velocity

        domain = states.get_domain()[:2]  # get x, y domain
        ...
        self.render_info(states, inputs)

        transformation_matrix = Transformation2D.translate_rotate_matrix(positions[:2], positions[2])

        # Draw the boat
        boat_shape = self.geometry.shape
        ...
        self.pygame.draw.polygon(self.screen, self.boat_color, boat_points)

        # Draw the input force arrow
        arrow_points = self.get_vector_force(inputs)
        ...
        self.pygame.draw.lines(self.screen, self.arrow_color, True, arrow_points, 3)
...
class DynamicCameraBoatRenderer(BoatRenderer):
    ...
    def render_boat(self, domain, boat_points):
        ...
        self.pygame.draw.polygon(self.screen, self.boat_color, points)

    def render_force(self, domain, inputs: Signal):
        force_direction = numpy.arctan2(inputs.values[1], inputs.values[0]) + self.heading_offset

        arrow_points = self.arrow_points(inputs.values)

        scale, offset = self.get_scale_and_offset(domain)
        offset = offset + numpy.array([0, self.geometry.thrust_offset]) * scale
        ...
        self.pygame.draw.lines(self.screen, self.arrow_color, True, points, width=3)

    def render_grid(self, domain, states: MechanicalStateSignal):
        positions = states.position
        ...
        start, end = self._get_grid_dimensions(domain, scale, coord)
        ...
        angle = -positions[2] - self.heading_offset
    ...
    def render_trajectory(self, domain, trajectory, states: MechanicalStateSignal):
        positions = states.position
        ...
        # draw the trajectory
        self.pygame.draw.lines(self.screen, self.trajectory_color, False, points, width=5)
    ...
    def render(self, states: MechanicalStateSignal, inputs: Signal, outputs: Signal):
        ...
        domain = states.get_domain()
        domain = domain + numpy.array([*positions, *velocities]).reshape(-1, 1) # Center the domain on the boat position
        domain = domain[:2] # get x, y domain

        self.render_grid(domain, states)

        # if trajectory is not None:
        #     self.render_trajectory(domain, trajectory, system.kinematic)

        self.render_info(states, inputs)
        self.render_boat(domain, self.geometry.shape)
        self.render_force(domain, inputs)
